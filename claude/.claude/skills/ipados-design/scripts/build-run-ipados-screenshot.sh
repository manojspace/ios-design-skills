...
if [[ $# -lt 3 ]]; then
  echo "Usage: build-run-ipados-screenshot.sh <repo> <scheme> <bundle-id> [device]" >&2
  exit 2
fi
...
SCHEME="$2"
BUNDLE_ID="$3"
DEVICE="${4:-iPad Pro 13-inch (M5)}"
DERIVED_DATA="${TMPDIR:-/tmp}/ipados-design-derived-data/${SCHEME}"
OUTPUT_DIR="${TMPDIR:-/tmp}/ipados-design-screenshots/${SCHEME}"
...
"$SCRIPT_DIR/capture-ipados-matrix.sh" "$BUNDLE_ID" "$OUTPUT_DIR" "$DEVICE"
